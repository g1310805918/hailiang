...
public class ServiceRequestController {
    ...
    public ResultUtil<List<SystemVersionVo>> getSystemVersion(@PathVariable("id") String id) {
        ...
        List<SystemVersionVo> voList = CollectionUtil.newArrayList();
        //操作系统对应的版本信息
        ...
        if (list.size() > 0 && list != null) {
            ...
            for (SysDictionary sysDictionary : list) {
                voList.add(new SystemVersionVo().setId(sysDictionary.getId().toString()).setContent(sysDictionary.getContent()));
            }
        }
        return resultUtil.AesJSONSuccess("SUCCESS", voList);
    }
    ...
    @ApiOperation(httpMethod = "POST", value = "用户工单升级")
    @RequestMapping(value = "/account-upgrade-work-order", method = RequestMethod.POST)
    public ResultUtil<String> accountUpgradeWorkOrder(AccountUpgradeWorkOrderReq accountUpgradeWorkOrderReq) {
        accountUpgradeWorkOrderReq = AESUtil.decryptToObj(accountUpgradeWorkOrderReq.getData(), AccountUpgradeWorkOrderReq.class);
        WorkOrder workOrder = workOrderService.getById(accountUpgradeWorkOrderReq.getWorkOrderId());
        if (workOrder != null) {
            //最近更新时间、工单升级状态、升级原因
            workOrder.setLastUpdateTime(DateUtil.now()).setUpgradeStatus(StatusCodeUtil.WORK_ORDER_UPGRADE_STATUS).setUpgradeReason(StrUtil.hasEmpty(accountUpgradeWorkOrderReq.getReason()) ? "" : accountUpgradeWorkOrderReq.getReason());
            boolean flag = workOrderService.updateById(workOrder);
            if (flag) {
                //异步发送【工单告警】消息至Manager工程师
                threadPoolTaskExecutor.execute(() -> sendMessageUtil.sendManagerWorkOrderUpgradeAlarm(workOrder));
            }
        }
        return resultUtil.AesJSONSuccess("升级成功", null);
    }


    @ApiOperation(httpMethod = "POST", value = "用户修改工单联系人信息")
    @RequestMapping(value = "/account-change-work-order-contact", method = RequestMethod.POST)
    public ResultUtil<String> accountChangeWorkOrderContact(ChangeWorkOrderContactInfoReq changeWorkOrderContactInfoReq) {
        changeWorkOrderContactInfoReq = AESUtil.decryptToObj(changeWorkOrderContactInfoReq.getData(), ChangeWorkOrderContactInfoReq.class);
        WorkOrder workOrder = workOrderService.getById(changeWorkOrderContactInfoReq.getWorkOrderId());
        if (workOrder != null) {
            if (changeWorkOrderContactInfoReq.getType() == 1) {
                //主要联系人信息
                workOrder.setMainContact(StrUtil.hasEmpty(changeWorkOrderContactInfoReq.getMainContact()) ? workOrder.getMainContact() : changeWorkOrderContactInfoReq.getMainContact());
                workOrder.setMainMobile(StrUtil.hasEmpty(changeWorkOrderContactInfoReq.getMainMobile()) ? workOrder.getMainMobile() : changeWorkOrderContactInfoReq.getMainMobile());
                workOrder.setMainEmail(StrUtil.hasEmpty(changeWorkOrderContactInfoReq.getMainEmail()) ? workOrder.getMainEmail() : changeWorkOrderContactInfoReq.getMainEmail());
            } else {
                //备用联系人信息
                workOrder.setStandbyContact(StrUtil.hasEmpty(changeWorkOrderContactInfoReq.getMainContact()) ? workOrder.getStandbyContact() : changeWorkOrderContactInfoReq.getMainContact());
                workOrder.setStandbyMobile(StrUtil.hasEmpty(changeWorkOrderContactInfoReq.getMainMobile()) ? workOrder.getStandbyMobile() : changeWorkOrderContactInfoReq.getMainMobile());
                workOrder.setStandbyEmail(StrUtil.hasEmpty(changeWorkOrderContactInfoReq.getMainEmail()) ? workOrder.getStandbyEmail() : changeWorkOrderContactInfoReq.getMainEmail());
            }
            //优先联系方式
            workOrder.setContactWay(changeWorkOrderContactInfoReq.getContactWay() != null ? workOrder.getContactWay() : changeWorkOrderContactInfoReq.getContactWay());
            workOrderService.updateById(workOrder);
        }
        return resultUtil.AesJSONSuccess("修改成功", null);
    }
